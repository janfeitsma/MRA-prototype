...
void determinePosition::pointsToPosition() {
	if( conf->getManualMode() ) {
		if( locList.size() < 1 ) { // at least one location should be available for  manual mode
			detPosSt newPos;
			newPos.pos = {0.0, 0.0, 0.0};
			newPos.goodEnough = false;
			newPos.score = 0.0;
			newPos.lastActive = 0;
			newPos.age = 0;
			newPos.amountOnFloor = 0;
			newPos.amountOffFloor = 0;
			newPos.numberOfTries = 0;
...
void determinePosition::goodEnough() {
	...
	for( size_t ii = 0; ii < candSelList.size(); ii++ ) {
		...
		printf("ii %2zu, index %2zu, last %3d, score %4.3f x %3.1f, y %3.1f, rz %3.1f, error %4.1f, mirror %d\n", ii,
				locListIndex, locList[locListIndex].lastActive, locList[locListIndex].score, candSelList[ii].xDelta,
	...
	if( locConfident ) {
		...
		if( candSelList.size() >= 1 ) {
			...
			if( candSelList[0].error < conf->getGoodEnough().newThreshold ) {
				...
				xDelta = abs(
						goodEnoughLoc.pos.x - ((double)rFloor->getXFloorRight() - locList[candSelList[0].index].pos.x));
				yDelta = abs(
						goodEnoughLoc.pos.y
